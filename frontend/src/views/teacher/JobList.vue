...
import { ElTable, ElTableColumn, ElButton, ElDialog, ElForm, ElFormItem, ElInput, ElMessage, ElDatePicker, ElSelect, ElOption, ElTabs, ElTabPane } from 'element-plus'
...
const fetchJobs = async () => {
  ...
  const data = await request.get('/teacher/job/page', {
    params: { page: pageInfo.page, pageSize: pageInfo.pageSize, status }
  })
...
const handleSubmit = async () => {
  if (form.id) {
    await request.put('/teacher/job', form)
    ElMessage.success('更新成功')
  } else {
    await request.post('/teacher/job', form)
...
const handlePublish = async (id) => {
  await request.put(`/teacher/job/${id}/publish`)
  ElMessage.success('发布成功')
  fetchJobs()
}

const handlePause = async (id) => {
  await request.put(`/teacher/job/${id}/pause`)
  ElMessage.success('已暂停')
  fetchJobs()
}

const handleClose = async (id) => {
  await request.put(`/teacher/job/${id}/close`)
  ElMessage.success('已关闭')
  fetchJobs()
}

const handleDelete = async (id) => {
  await request.delete(`/teacher/job/${id}`)
